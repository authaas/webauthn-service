...
`credentialId` uniqueness is a property this store imposes, not one
authenticators provide. Neither WebAuthn nor CTAP specifies how an
authenticator generates a credential ID; vendors use random values, wrapped
private keys, and deterministic derivations from the RP ID and user handle, and
no scheme guarantees that two credentials never share an ID. A registration
presenting a `credentialId` the store already holds is refused, whatever
authenticator produced it. An authenticator that then produces a different ID
for a new credential registers on the next attempt; one that cannot has no
credential this system can hold, and is unusable with it.
